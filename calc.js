...
lengths = matix.range(length_min, 10000)
...
for(i = 0; i < lengths.length; i++) {
    length = lengths[i]
    width_max = length * 7
    widths = matix.range(width_min, width_max)
    ...
    for(j = 0; j < widths.length; j++) {
        ...
        if(length > 7 * width || width > 7 * length ) { continue }
        ...
        //put in all verifications' 1s and 0s
        // matrix[i].push([matix.verification4a(), matix.verification4b(), matix.verification5(), matix.verification6(), matix.verification7()])
    }
}
