...
for(i = 0; i < lengths.length; i++) {
    ...
    for(j = 0; j < widths.length; j++) {
        ...
        if(matix.verification5() === 0) {
            v5_matrix[i].push(0)
        } else {
            v5_matrix[i].push(matix.get_volume())
...
const csvWriter = createCsvWriter({
    header: header,
    path: './matrices/ex3/ex3_v5_matrix.csv'
})

csvWriter.writeRecords(v5_matrix)
    .then(() => {
        console.log('...v5_matrix.csv updated!');
    });