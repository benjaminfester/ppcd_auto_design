...
for(i = 0; i < lengths.length; i++) {
    ...
    for(j = 0; j < widths.length; j++) {
        ...
        if(matix.verification4a() === 0) {
            v4a_matrix[i].push(0)
        } else {
            v4a_matrix[i].push(matix.get_volume())
...
const csvWriter = createCsvWriter({
    header: header,
    path: './matrices/ex3/ex3_v4a_matrix.csv'
})

csvWriter.writeRecords(v4a_matrix)
    .then(() => {
        console.log('...v4a_matrix.csv updated!');
    });